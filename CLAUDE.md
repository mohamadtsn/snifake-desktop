...
- `src/components/AboutSection.tsx` — the About panel: version (read from the bundle via `getVersion()`), package identifier, source and license, plus a **Check for updates** button. It calls `checkUpdate()` rather than `findUpdate()` because a check somebody pressed has to distinguish "you are current" from "the server was unreachable", where the silent one on launch must not. Sits at the end of the scrolling column, not in the fixed furniture, so it never costs the switch any room.
