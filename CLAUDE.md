...
- `src/components/StatusControl.tsx` — status and primary action as **one** control. The top row *is* the Start/Stop button (orb + state label + morphing action chip; Stop stays available during `starting`); below a hairline sit the read-only Listen/Upstream/SNI route rows, deliberately outside the button so reading them can't toggle the proxy. The orb + label are keyed on `state` so they remount, but the button is not — focus survives a state change.
...
- `src/components/ConnectionSection.tsx` — the 5 config fields inside a `Disclosure`, laid out as host+port pairs on one line each so the control mirrors the shape of the value. Inputs are `dir="ltr"` and left-aligned (they hold hostnames/IPs/ports, never prose) and every field keeps a visible label. Exposes `getValue()`/`validate()` via `forwardRef`/`useImperativeHandle` as `ConfigFormHandle`, and only shows "Save changes" while dirty.
